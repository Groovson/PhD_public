...
## Filters #######################################################################################
hpt_ukb_sub2 <- hpt_ukb_sub1 |> filter(!is.na(date_of_cmri))           # Those with no CMRI Data
...
## Diastolic_BP
model_mx2 <- lmer(diastolic_bp ~ year2 + (year2|n_eid_14631), hpt_ukb_sub2)    # Gener
summary(model_mx2)

## Pulse Pressure
model_mx3 <- lmer(pp ~ year2 + (year2|n_eid_14631), hpt_ukb_sub2)    # Gener
summary(model_mx3)
...
rand_df <- rand_df |> mutate(Decline_SBP5_Cat = as_factor(case_when(
  Decline_SBP5 < -5 ~ "Decline",
  Decline_SBP5 >= -5 & Decline_SBP5 < 5 ~ "Stable",
  Decline_SBP5 >= 5 ~ "Increase")))

rand_df <- rand_df |> mutate(Decline_SBP7_Cat = as_factor(case_when(
  Decline_SBP7 < -5 ~ "Decline",
  Decline_SBP7 >= -5 & Decline_SBP7 < 5 ~ "Stable",
  Decline_SBP7 >= 5 ~ "Increase")))

rand_df <- rand_df |> mutate(Decline_SBP10_Cat = as_factor(case_when(
...
model_lm <- lm(pwv_aix ~ Decline_SBP5_Cat +age_SSD + sex + education + hpt_dx_hx + bmi_cats + ldl_Cat + smoking_status + t2d_hes2022, data = hpt_ukb_sub3)
summary(model_lm)
confint(model_lm, level = 0.95)
